...
export default function PhotoPlaceholder({
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 my-8 flex flex-col items-center justify-center text-center">
          <div className="relative mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border border-dashed border-white/20 bg-white/[0.02] text-white/30 font-mono">
            <span className="text-xs uppercase tracking-widest">+</span>
          </div>
          <p className="text-base font-semibold tracking-tight text-white md:text-lg">{title}</p>
          {subtitle && <p className="mt-1.5 max-w-xs text-xs leading-relaxed text-white/50">{subtitle}</p>}
        </div>
        <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-4 text-[11px] text-white/40">
          <span className="font-mono tracking-wider uppercase">{specs || "High resolution required"}</span>
          <span className="text-[10px] uppercase tracking-widest text-[#DFB76C]/70">UCS Archive</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={`group relative flex flex-col justify-between overflow-hidden rounded-[2rem] border border-[var(--ucs-border)] bg-[var(--ucs-bg-card)] p-6 text-[var(--ucs-text-primary)] shadow-xl transition-all duration-500 hover:border-[var(--ucs-gold)] ${className}`}
    >
      ...
      <div className="relative z-10 flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-2 rounded-full border border-[var(--ucs-gold-badge-border)] bg-[var(--ucs-gold-badge-bg)] px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.25em] text-[var(--ucs-gold-text)]">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[var(--ucs-green)]" />
          Photo Needed
        </span>

        <div className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--ucs-border)] bg-[var(--ucs-toggle-bg)] text-[var(--ucs-text-muted)] transition group-hover:border-[var(--ucs-gold)] group-hover:text-[var(--ucs-gold-text)]">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75">
            <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" />
            <circle cx="12" cy="13" r="4" />
          </svg>
      ...
      <div className="relative z-10 my-8 flex flex-col items-center justify-center text-center">
        <div className="relative mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border border-dashed border-[var(--ucs-border)] bg-[var(--ucs-toggle-bg)] text-[var(--ucs-text-muted)] font-mono">
          <span className="text-xs uppercase tracking-widest">+</span>
        </div>
        <p className="text-base font-semibold tracking-tight text-[var(--ucs-text-primary)] md:text-lg">
          {title}
        </p>
        {subtitle && (
          <p className="mt-1.5 max-w-xs text-xs leading-relaxed text-[var(--ucs-text-muted)]">
            {subtitle}
          </p>
        )}
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between border-t border-[var(--ucs-border-subtle)] pt-4 text-[11px] text-[var(--ucs-text-subtle)]">
        <span className="font-mono tracking-wider uppercase">
          {specs || "High resolution required"}
        </span>
        <span className="text-[10px] uppercase tracking-widest text-[var(--ucs-gold-text)]">
          UCS Archive
        </span>
      </div>
    </div>
